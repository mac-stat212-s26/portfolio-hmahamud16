...
-   Relate the intro paragraph: "Scientists declared that 2015 was Earth’s hottest year on record..." to the *design* of the visualization. In particular, based on the intro paragraph,
    -   What key message/claim does NYTimes want readers to be able to explore?
        -   The claim that 2015 was the hottest year on record.
    -   How did this goal inform what information is displayed in the visualization?
        - The data should ideally include ranges on normal vs. hottest tempuratures, in order for readers to decide themselves if that year in that given area was really the hottest on record.

**Aesthetic Mapping**

-   What specific variables (from the data codebook) underlie the visualization?
    - The normal low and normal high, as well as axis which indicate the normal temperature and the record for that given day. 
-   How do these variables map to aesthetics of the visual elements, eg, position, size, shape, and color of glyphs?
    - They map them in the form of bars for each individual day.
...
ggplot(weather, aes(x = dateInYear)) +
  ...
  labs(title = "SFO Weather Ranges in 2011", y = "Value Range")
```

::: {.callout-tip title="Keyboard Shortcuts"}
As you work on this plot, try to use some new [keyboard shortcuts](https://hash-mac.github.io/stat212site-sp26/notes/keyboard.html).
...
ggplot(aes(x = dateInYear)) +
  ...
  geom_linerange(aes(ymin = NormalLow, ymax = NormalHigh), color = "#C8B8BA", linewidth = 1) +
  geom_linerange(aes(ymin = Low, ymax = High), color = "#A90248", linewidth = 1) +
  geom_vline(data = last_day_months,
             aes(xintercept = dateInYear), 
             color = "red", 
             linetype = "dashed", 
             size = 0.2) +
  labs(title = "SFO Weather Ranges in 2011", y = "Value Range", x = "Month")
...
```{r}
# Calculate midpoints for each month
...
weather |> 
  ...
  geom_vline(data = last_day_months,
             ...
             size = 0.2) +
  scale_x_continuous(
    breaks = month_midpoints$midpoint,
    labels = month.abb
  ) +
  labs(title = "SFO Weather Ranges in 2011", y = "Value Range", x = "Month")
```
...
weather |> 
ggplot(aes(x = dateInYear, y = CulmPrec), fill = "#ebeae2") + 
  geom_line(color = "#32a3d8",fill = "#ebeae2")
```

```{r}
weather |> 
  ggplot(aes(x = dateInYear, y = CulmPrec)) + 
  geom_area(fill = "#ebeae2", color = "#32a3d8", linewidth = 0.5) +
  geom_text(data = weather |> 
              filter(CulmPrec > 1.4),  # Only label peaks above 1.4
            aes(label = round(CulmPrec, 2)),
            vjust = -0.8, 
            size = 3.5,
            color = "black") +
  geom_point(data = weather |> 
               filter(CulmPrec > 1.4),  # Only show triangles at major peaks
             shape = 25,  
             fill = "black",
             size = 2) +

...
```{r}
# Steps:
# 1. Create a character storage container with length = ncol(mtcars)
# 2. Loop over each column index with seq_along()
# 3. Extract the column with [[i]] and call class() on it
# 4. Store the result; name the output vector using colnames()
...
```{r}
# Function returning summary stats as a 1-row data frame
...
# Select numeric columns from diamonds
...
# for loop approach
results <- vector("list", length = ncol(diamonds_num))
for (i in seq_along(diamonds_num)) {
    results[[i]] <- summ_stats(diamonds_num[[i]])
}
bind_rows(results) |>
    mutate(variable = colnames(diamonds_num), .before = 1)

# map() is more concise and readable; the for loop requires manual storage management.
```
...
```{r}
# Example 1: Different string, start, and end for each iteration
example1 <- tibble(
    string = c("January", "February", "March"),
    start  = c(1, 1, 1),
    end    = c(3, 3, 3)
)
pmap_chr(example1, str_sub)
# Returns: "Jan", "Feb", "Mar"

# Example 2: Fixed string, different start and end positions
example2 <- tibble(
    start = c(1, 5, 8),
    end   = c(4, 7, 10)
)
pmap_chr(example2, str_sub, string = "Hello World!")
# Returns: "Hell", "o W", "orl"
```
...
# use pmap()
# pmap() iterates over rows of df_arg_combos, passing each row's columns as
# named arguments to fit_model(). The fixed `data` argument is passed separately.
fitted_models <- pmap(df_arg_combos, fit_model, data = diamonds)
fitted_models
```

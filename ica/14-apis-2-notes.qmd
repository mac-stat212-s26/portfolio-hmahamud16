...
census_api_key <- readLines("census_api_key") |> trimws()
```
...
resp # inspect status and content-type before parsing
```
...
```{r census_json}
# Peek at the raw response to diagnose content-type issues
...
# 1. Get the board game name (the "value" attribute of the <name> node)
resp |> xml_find_all("item/name") |> xml_attr("value")

# 2. Get the board game ID number (the "id" attribute of each <item> node)
...
# The copyright key (top-level)
resp_pt$copyright

# Number of hits within the meta object (nested: response -> meta -> hits)
resp_pt$response$meta$hits

# Abstracts of the articles (nested: response -> docs -> abstract)
resp_pt$response$docs$abstract

# Leading paragraphs of the articles (nested: response -> docs -> lead_paragraph)

...
str_c("Month ", seq_along(month.name), ": ", month.name)
```
...
models <- vector("list", length = length(cut_levels))

for (i in seq_along(cut_levels)) {
    data_subset <- diamonds |> filter(cut == cut_levels[i])
    models[[i]] <- lm(price ~ carat, data = data_subset)
}

names(models) <- cut_levels
models
```
...
fit_model <- function(data, mod_formula, remove_outliers) {
    ...
    if (remove_outliers) {
        data <- remove_outliers(data, what_var = carat, sd_thresh = 3)
    }

    # Use mod_formula as the first argument of lm()
    lm(mod_formula, data = data)
}
```
...
```{r}
fitted_models <- vector("list", length = nrow(df_arg_combos))

for (i in seq_len(nrow(df_arg_combos))) {
    fitted_models[[i]] <- fit_model(
        data            = diamonds,
        mod_formula     = df_arg_combos$mod_formula[[i]],
        remove_outliers = df_arg_combos$remove_outliers[i]
    )
}

...
thursdayquote <- 'what if i wanted to say "its thursday" on my own?'
# how would i put an apostrophe on a word like its inside double quotes?
```
...
# without it, an error occurs that we used hex digits in a character string. 
```

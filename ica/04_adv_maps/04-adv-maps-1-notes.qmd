...
```
```{r}
transform_and_plot(nc, new_crs = "EPSG:5940")

transform_and_plot(nc, new_crs = "EPSG:20353")
```
...
The viridis function creates color-blind accessible color palettes, and also create palettes that contain colors that are visibly different from one another. Secondly, the difference between b, c, and d is that they have different color pairing between each other, which helps enhance visibility.
...
png("../04_adv_maps/figures/tc_map_zoom.png", width = 250, height = 400)

...
- The publication date: `.thumbnail-teaser__date`
...
Planning this exercise felt manageable once I broke it down. The key insight was that each page has the same HTML structure, so a single scraping function could be reused across pages by varying only the page number in the URL. Iteration with `map()` made the multi-page collection concise. The trickiest part was figuring out the URL pattern for pagination and removing the date from the abstract text with a regex.
:::
...
```{r scrape_nih_pages}
# Function to scrape one page of NIH news releases
# URL pattern: page 1 = base URL, pages 2+ add ?page=1, ?page=2, etc. (0-indexed)
scrape_nih_page <- function(page_num) {
    if (page_num > 1) Sys.sleep(1) # be polite even without a required Crawl-delay

    url <- if (page_num == 1) {
        "https://www.nih.gov/news-events/news-releases"
    } else {
        str_c("https://www.nih.gov/news-events/news-releases?page=", page_num - 1)
    }

    page_html <- read_html(url)

    # Scrape by article container so all three fields are guaranteed to align
    articles <- page_html |> html_elements(".thumbnail-teaser")

    tibble(
        title = articles |>
            html_element(".thumbnail-teaser__heading") |>
            html_text(trim = TRUE),
        date = articles |>
            html_element(".thumbnail-teaser__date") |>
            html_text(trim = TRUE),
        abstract = articles |>
            html_element(".thumbnail-teaser__description") |>
            html_text(trim = TRUE) |>
            # Remove the leading date (e.g., "April 14, 2026") from the abstract
            str_remove("^[A-Z][a-z]+ \\d{1,2}, \\d{4}\\s*")
    )
}

# map() approach: scrape pages 1–5 and combine into one data frame
nih_news <- map(1:5, scrape_nih_page) |>
    list_rbind()

nih_news

# for loop approach (same result)
nih_news_loop <- vector("list", length = 5)
for (p in 1:5) {
    nih_news_loop[[p]] <- scrape_nih_page(p)
}
nih_news_loop <- bind_rows(nih_news_loop)
nih_news_loop
```
